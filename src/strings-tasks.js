...
function removeFirstOccurrences(str, value) {
  ...
  if (a === -1) {
    ...
    ans = str.slice(0, a) + str.slice(a + value.length);
  }
  return ans;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.length; i += 1) {
    if (sentence[i] !== ' ' && i !== sentence.length - 1) {
      ...
      ans = currAns.length > ans.length ? currAns : ans;
      currAns = '';
    }
  }
  return ans;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      ...
      ans += String.fromCharCode(str[i].charCodeAt(0) + 13);
    } else if (
      (str[i].charCodeAt(0) >= 78 && str[i].charCodeAt(0) <= 90) ||
      (str[i].charCodeAt(0) >= 110 && str[i].charCodeAt(0) <= 122)
    ) {
      ans += String.fromCharCode(str[i].charCodeAt(0) - 13);
    } else {
      ans += str[i];
